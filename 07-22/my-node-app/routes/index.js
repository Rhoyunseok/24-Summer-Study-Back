...
router.get("/", function (req, res, next) {
  console.log(
    "http://localhost:3000/ 라우팅 메소드의 콜백함수가 호출되었습니다."
  );

  console.log(
    "사용자클라이언트 정보를 req(HttpRequest) 객체에서 조회가능합니다,",
    req
  );
  ...
  console.log(
    "서버측에서 웹브라우저를 응답처리를 위한 res(HttpResponse)객체 조회",
    res
  );

  console.log(
    "res.render()메소드를 통해 메인 뷰파일(index.ejs)파일이 바로 호출됩니다..."
  );
