//프로젝트에 설치된 노드패키지를 참조한다.
//node.js backend에서 require 예약어를 이용해 설치된 패키지를 참조한다.
const moment = require('moment');

//환경설정파일에서 환경변수를 가져오기위해 dotenv 패키지를 참조한다.
const env = require('dotenv');

//프로젝트 루트에 있는 .env파일에 환경변수정보를 메모리상에 CPU프로세스에서 접근가능하게 구성해준다.
env.config();

// console 객체는 node framework 자체에서 제공하는 내장 객체
// 헷갈리지 마세요. console 객체는 웹 브라우저 개발자도구 console의 로그와 다릅니다.
console.log("index.js 모듈이 시작되었습니다.");

var toDate = Date();
var toDate2 = Date.now();

// 순수 자바스크립트 날짜 데이터는 기본 숫자형으로 표시됩니다.
console.log("현재 일시를 출력합니다. 순수 자바스크립트 :  " + toDate );

console.log("현재 일시를 출력합니다. 순수 자바스크립트 :  " + toDate2 );
